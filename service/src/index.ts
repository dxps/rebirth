...
const usernameUniqueConflictResponse: ApiErrorResponse = {
	error: {
		code: 'unique_conflict',
	...
	for (const property of ['cause', 'error', 'originalError']) {
		if (
			isPostgresErrorWithCode(
				getErrorProperty(error, property),
				code,
				seen,
			)
		) {
...
function isPostgresErrorWithConstraint(
	...
	if (
		getErrorProperty(error, 'constraint_name') === constraint ||
		getErrorProperty(error, 'constraint') === constraint
	) {
		return true
	}
...
function canManageData(user: User): boolean {
	return (
		...
		hasPermission(user, PermissionName.Editor)
	)
}
...
function canViewData(user: User): boolean {
	return (
		canCreateManagedData(user) || hasPermission(user, PermissionName.Viewer)
	)
}
...
function canManageOwnedRecord(
	...
	return (
		canManageData(user) ||
		(canManageOwnData(user) && record.ownerUserId === user.id)
	)
}
...
async function requireManagedDataCreator(
	request: Request,
): Promise<Response | User> {
	const user = await requireAuthenticatedUser(request)
...
async function requireSecurityManager(
	request: Request,
): Promise<Response | User> {
	const user = await requireAuthenticatedUser(request)
...
const server = Bun.serve({
	...
	async fetch(request) {
		...
		if (
			request.method === 'POST' &&
			url.pathname === apiRoutes.authLogout
		) {
		...
		if (
			request.method === 'PUT' &&
			url.pathname === apiRoutes.userPassword
		) {
			const authenticatedUser = await requireAuthenticatedUser(request)
		...
		if (request.method === 'PUT' && url.pathname === apiRoutes.userInfo) {
			const authenticatedUser = await requireAuthenticatedUser(request)
			...
			try {
				...
				if (!isUpdateUserInfoInput(input)) {
					return Response.json(
						{
				...
				if (isPostgresErrorWithCode(error, uniqueConflictErrorCode)) {
					if (
						...
						return Response.json(usernameUniqueConflictResponse, {
							headers: jsonHeaders,
							status: 409,
						})
					}
		...
		if (
			request.method === 'GET' &&
			url.pathname === apiRoutes.permissions
		) {
			try {
		...
		if (
			request.method === 'GET' &&
			url.pathname === apiRoutes.auditEvents
		) {
			...
			try {
				...
				if (existingUserCount > 0) {
					const authenticatedUser =
						await requireSecurityManager(request)
		...
		if (
			request.method === 'GET' &&
			url.pathname === apiRoutes.accessLevels
		) {
		...
		if (
			request.method === 'GET' &&
			url.pathname === apiRoutes.attributeTemplates
		) {
			const authenticatedUser = await requireDataViewer(request)
			...
			try {
				...
				const createdAttributeTemplate = await createAttributeTemplate(
					authenticatedUser.id,
					{
						...
						defaultValue:
							normalizeDefaultValue(input.defaultValue) ?? null,
						description: input.description.trim(),
				...
				if (
					isPostgresErrorWithConstraint(
						error,
						attributeTemplateUniqueConstraint,
					)
				) {
					return Response.json(
						attributeTemplateUniqueConflictResponse,
						{
							headers: jsonHeaders,
							status: 409,
						},
		...
		if (
			request.method === 'GET' &&
			url.pathname === apiRoutes.entityTemplates
		) {
			const authenticatedUser = await requireDataViewer(request)
				...
				const requestedPageSize = Math.max(
					Number.parseInt(
						url.searchParams.get('pageSize') ??
							String(defaultEntitiesPageSize),
						10,
					) || defaultEntitiesPageSize,
					1,
				)
		...
		const attributeTemplateMatch =
			/^\/attribute-templates\/([0-9a-f-]+)$/i.exec(url.pathname)
		const entityMatch = /^\/entities\/([0-9a-f-]+)$/i.exec(url.pathname)
		const entityTemplateMatch = /^\/entity-templates\/([0-9a-f-]+)$/i.exec(
			url.pathname,
		)
		...
		if (request.method === 'PATCH' && accessLevelMatch) {
			...
			try {
				...
				if (id <= builtInAccessLevelMaxId && input.name !== undefined) {
					const existingAccessLevel = (await listAccessLevels()).find(
		...
		if (request.method === 'PATCH' && attributeTemplateMatch) {
			...
			try {
				...
				if (
					!isAttributeTemplateId(id) ||
					!isUpdateAttributeTemplateInput(input)
				) {
					return Response.json(
						{
				...
				const existingAttributeTemplate = (
					await listAttributeTemplates()
				).find((attributeTemplate) => attributeTemplate.id === id)
		...
		if (request.method === 'DELETE' && attributeTemplateMatch) {
			...
			try {
				...
				const deletedAttributeTemplate =
		...
		if (request.method === 'PATCH' && entityTemplateMatch) {
			...
			try {
				...
				if (
					!isEntityTemplateId(id) ||
					!isUpdateEntityTemplateInput(input)
				) {
					return Response.json(
						{
				...
				const existingEntityTemplate = (
					await listEntityTemplates()
				).find((entityTemplate) => entityTemplate.id === id)
